/** Lógica de línea de venta Prendas vs Tenis — solo cuenta Winston (SUC-WIN). */
...
export type LineaVentaWinston = 'prendas' | 'tenis';
...
export function dividirDetallesPorLinea<T extends { prenda_id: string; prenda_nombre?: string }>(
  ...
  for (const d of detalles) {
    if (esPrendaTenis(d.prenda_id, d.prenda_nombre)) {
      tenis.push(d);
    } else {
      prendas.push(d);
    }
  }
  return { prendas, tenis };
}
