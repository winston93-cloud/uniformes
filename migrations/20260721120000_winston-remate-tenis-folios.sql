-- Winston: línea remate_tenis con folios rt0001…
-- Misma lógica que tenis (wt…), prenda REMATE TENIS.
...
BEGIN
  ...
  IF v_codigo = 'SUC-WIN' THEN
    IF p_linea_venta = 'tenis' THEN
      prefijo := 'wt';
    ELSIF p_linea_venta = 'remate_tenis' THEN
      prefijo := 'rt';
    ELSIF p_linea_venta = 'prendas' THEN
      prefijo := 'wu';
    ELSE
      RAISE EXCEPTION 'linea_venta requerida en Winston: prendas, tenis o remate_tenis';
    END IF;
...
COMMENT ON FUNCTION public.generar_folio_pedido(UUID, VARCHAR) IS
  'Matriz: PED-{CODIGO}-YYYYMM-0001. Winston: wu (prendas), wt (tenis), rt (remate tenis).';

-- Solo actualizar el mensaje de validación en crear_pedido_atomico
CREATE OR REPLACE FUNCTION public.crear_pedido_atomico(
  p_tipo_cliente character varying,
  p_cliente_nombre character varying,
  p_sucursal_id uuid,
  p_usuario_id uuid DEFAULT NULL::uuid,
  p_alumno_id text DEFAULT NULL::text,
  p_externo_id uuid DEFAULT NULL::uuid,
  p_estado character varying DEFAULT 'PENDIENTE'::character varying,
  p_notas text DEFAULT NULL::text,
  p_detalles jsonb DEFAULT '[]'::jsonb,
  p_linea_venta character varying DEFAULT NULL::character varying
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_codigo_sucursal = 'SUC-WIN' AND (
    p_linea_venta IS NULL
    OR p_linea_venta NOT IN ('prendas', 'tenis', 'remate_tenis')
  ) THEN
    RAISE EXCEPTION 'linea_venta requerida en Sucursal Winston (prendas, tenis o remate_tenis)';
  END IF;
