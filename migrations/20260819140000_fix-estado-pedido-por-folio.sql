-- Fix: estado PENDIENTE no debe heredarse del carrito mixto a folios sin pendientes (wu/wt/rt).
-- Antes: sin pendientes en el folio pero p_estado=PENDIENTE → quedaba PENDIENTE.
-- Ahora: el estado depende solo de las partidas de ese folio.
...
BEGIN
  ...
  v_estado_final := CASE WHEN v_any_pending THEN 'PENDIENTE' ELSE 'COMPLETADO' END;
...
-- Reparar folios marcados PENDIENTE sin partidas pendientes (ej. tenis wt… por carrito mixto).
