-- Winston (SUC-WIN): folios wu0001 (prendas) y wt0001 (tenis); línea en pedidos y cortes.

ALTER TABLE public.pedidos
  ADD COLUMN IF NOT EXISTS linea_venta VARCHAR(20);

ALTER TABLE public.cortes
  ADD COLUMN IF NOT EXISTS linea_venta VARCHAR(20);

COMMENT ON COLUMN public.pedidos.linea_venta IS 'prendas | tenis — solo Sucursal Winston; null en matriz.';
COMMENT ON COLUMN public.cortes.linea_venta IS 'prendas | tenis — corte separado por línea en Winston.';

DROP FUNCTION IF EXISTS public.generar_folio_pedido(UUID);

CREATE OR REPLACE FUNCTION public.generar_folio_pedido(
  p_sucursal_id UUID,
  p_linea_venta VARCHAR DEFAULT NULL
)
RETURNS TEXT
LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_codigo = 'SUC-WIN' THEN
    IF p_linea_venta = 'tenis' THEN
      prefijo := 'wt';
    ELSIF p_linea_venta = 'prendas' THEN
      prefijo := 'wu';
    ELSE
      RAISE EXCEPTION 'linea_venta requerida en Winston: prendas o tenis';
    END IF;
...
COMMENT ON FUNCTION public.generar_folio_pedido(UUID, VARCHAR) IS
  'Matriz: PED-{CODIGO}-YYYYMM-0001. Winston: wu0001 (prendas) o wt0001 (tenis).';

GRANT EXECUTE ON FUNCTION public.generar_folio_pedido(UUID, VARCHAR) TO authenticated;
GRANT EXECUTE ON FUNCTION public.generar_folio_pedido(UUID, VARCHAR) TO anon;

DROP FUNCTION IF EXISTS public.crear_pedido_atomico(VARCHAR, VARCHAR, UUID, UUID, TEXT, UUID, VARCHAR, TEXT, JSONB);
...
BEGIN
  ...
  SELECT codigo INTO v_codigo_sucursal FROM public.sucursales WHERE id = p_sucursal_id;
  IF v_codigo_sucursal = 'SUC-WIN' AND p_linea_venta IS NULL THEN
    RAISE EXCEPTION 'linea_venta requerida en Sucursal Winston (prendas o tenis)';
  END IF;
  ...
  v_folio := public.generar_folio_pedido(p_sucursal_id, p_linea_venta);

  INSERT INTO public.pedidos (
    tipo_cliente, cliente_nombre, sucursal_id, usuario_id,
    alumno_id, externo_id, estado, subtotal, total, notas, folio, linea_venta
  ) VALUES (
    ...
    CASE WHEN v_codigo_sucursal = 'SUC-WIN' THEN p_linea_venta ELSE NULL END
