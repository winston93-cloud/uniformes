-- Quitar descuento de INSUMOS al crear pedidos.
-- A partir de esta migración, crear_pedido_atomico:
-- - Descuenta SOLO stock de prendas (costos) para cantidad_con_stock
-- - (Si existe) descuenta costo_ubicaciones desde la ubicación con menor stock
-- - NO descuenta insumos.stock
-- - NO descuenta insumo_ubicaciones
-- - Mantiene snapshot_insumos_pedido para trazabilidad (receta × cantidad_con_stock)
...
BEGIN
  ...
  -- Calcular totales (subtotal = sum(cantidad_total * precio_unitario))
  ...
  -- Insertar detalles + descontar SOLO costos (y costo_ubicaciones si existe)
  FOR v_detalle IN SELECT * FROM jsonb_array_elements(p_detalles)
  LOOP
    v_cantidad_total := COALESCE((v_detalle->>'cantidad')::INTEGER, 0);
    ...
    IF v_cantidad_con_stock > 0 THEN
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION
          'Stock insuficiente para % talla % al crear pedido. Requerido: %.',
          v_prenda_nombre,
          v_talla_nombre,
          v_cantidad_con_stock;
      END IF;

      -- Si existe reparto por ubicaciones de costos, descontar desde la de menor stock
      ...
      -- Snapshot de insumos (SOLO trazabilidad). No se descuenta stock de insumos aquí.
...
GRANT EXECUTE ON FUNCTION crear_pedido_atomico(VARCHAR, VARCHAR, UUID, UUID, UUID, UUID, VARCHAR, TEXT, JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION crear_pedido_atomico(VARCHAR, VARCHAR, UUID, UUID, UUID, UUID, VARCHAR, TEXT, JSONB) TO anon;

COMMENT ON FUNCTION crear_pedido_atomico(VARCHAR, VARCHAR, UUID, UUID, UUID, UUID, VARCHAR, TEXT, JSONB) IS
  'Crea pedidos. Descuenta SOLO costos (y costo_ubicaciones si existe). NO descuenta insumos ni insumo_ubicaciones; snapshot_insumos_pedido solo para trazabilidad.';
